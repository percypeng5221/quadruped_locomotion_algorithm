...
class ManipLoco_Policy():
    ...
    def print_and_refresh(self):
        # Construct the output as a single string with newlines separating the lines
        output = (
            f"self.env.ee_pos: {self.env.ee_pos}, self.env.curr_ee_goal_cart_world: {self.env.curr_ee_goal_cart_world}\n"
            f"self.env.ee_orn: {self.env.ee_orn}, self.env.ee_goal_orn_quat: {self.env.ee_goal_orn_euler }\n"
            f"self.env.commands: {self.env.commands}, self.env.base_lin_vel: {self.env.base_lin_vel}, "
            f"self.env.base_ang_vel: {self.env.base_ang_vel}"
        )

        # Use carriage return '\r' and ANSI escape codes to overwrite the previous content
        sys.stdout.write("\033[F\033[F\033[F\033[K" * 3)  # Move up and clear 3 lines
