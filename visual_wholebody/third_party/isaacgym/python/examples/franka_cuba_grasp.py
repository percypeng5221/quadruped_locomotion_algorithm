...
custom_parameters = [
    ...
    {"name": "--num_envs", "type": int, "default": 9, "help": "Number of environments to create"},
]
...
for i in range(num_envs):
    ...
    camera_offset = gymapi.Vec3(0.0, 0, -0.5)
    camera_rotation = gymapi.Quat.from_axis_angle(gymapi.Vec3(1, 1, 1), np.deg2rad(250))
    body_handle = gym.get_actor_rigid_body_handle(env, franka_handle, hand_idx)
    gym.attach_camera_to_body(cam_handle, env, body_handle, gymapi.Transform(camera_offset, camera_rotation), gymapi.FOLLOW_TRANSFORM)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    depth_frame_np = depth_image.cpu().numpy()
