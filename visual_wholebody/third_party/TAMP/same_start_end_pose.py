...
# mass buffer
mass_list = torch.zeros(num_envs).to(device)
...
for env_i in range(num_envs):
    ...
    for box_i in range(num_boxs):
        ...
        for b in range(len(body_props)):
            tmp_mass = np.random.uniform(0.2, 2.2)
            body_props[b].mass = tmp_mass
            mass_list[env_i] = tmp_mass
        gym.set_actor_rigid_body_properties(env, box_handle, body_props, True)
...
# prepare pose generator
pose_generator = PoseGenerator(num_envs, 
                               table_pose_x=table_pose.p.x, 
                               table_dims_z=table_dims.z + 0.2, 
                               table_pose_y=table_pose.p.y, 
                               box_size=box_size)
...
action_cost = torch.zeros(num_envs).to(device)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    # get box states
    box_pos = rb_states[[sublist[0] for sublist in box_idxs], :3]
    box_rot = rb_states[[sublist[0] for sublist in box_idxs], 3:7]
    ...
    to_place_pos = position - hand_pos
    ...
    # generate new place points:
    generate_idx = torch.where((action_status == 4) & (to_init_dist < 0.02), 
                               torch.tensor([True] * num_envs).to(device),
                               torch.tensor([False] * num_envs).to(device))
    position, quat = pose_generator.generate(index=generate_idx)
    for env_i in range(num_envs):
        position[env_i, :] = position[0, :]
        quat[env_i, :] = quat[0, :]
    ...
    for env_i in range(num_envs):
        if action_status[env_i] == 0:
            ...
            goal_pos[env_i] = position[env_i]
            goal_rot[env_i] = quat_mul(down_q, quat_conjugate(quat[env_i]))
            close_gripper[env_i] = True
    ...
    action_cost += torch.where(action_status == 2, torch.norm(joint_dpose, dim=-1), torch.zeros(num_envs).to(device))
    print("action_cost: ", action_cost)
    print("mass_list: ", mass_list)
    
    # Calculate the mean of each tensor
    action_cost_mean = torch.mean(action_cost)
    mass_list_mean = torch.mean(mass_list)

    # Subtract the mean from each tensor (center them)
    action_cost_centered = action_cost - action_cost_mean
    mass_list_centered = mass_list - mass_list_mean

    # Calculate the covariance between the tensors
    covariance = torch.sum(action_cost_centered * mass_list_centered)

    # Calculate the standard deviations of each tensor
    action_cost_std = torch.sqrt(torch.sum(action_cost_centered ** 2))
    mass_list_std = torch.sqrt(torch.sum(mass_list_centered ** 2))

    # Calculate the Pearson correlation coefficient
    correlation_coefficient = covariance / (action_cost_std * mass_list_std)
