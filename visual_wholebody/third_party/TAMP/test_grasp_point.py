...
for env_i in range(num_envs):
    ...
    for box_i in range(num_boxs):
        ...
        for b in range(len(body_props)):
            body_props[b].mass = np.random.uniform(0.2, 0.8)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    action_cost += torch.where(action_status != 4, torch.norm(joint_dpose, dim=-1), torch.zeros(num_envs).to(device))
