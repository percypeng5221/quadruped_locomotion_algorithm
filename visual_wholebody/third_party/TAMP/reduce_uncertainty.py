...
from rollout_parameter import rolloutPara
from utils import calculateTrajectorySimilarity
...
def cube_grasping_yaw(q, corners, add_pi_half=None):
    """ returns horizontal rotation required to grasp cube with optional additional rotation tensor """
    rc = quat_rotate(q, corners)
    
    # Calculate the base yaw angle
    yaw = (torch.atan2(rc[:, 1], rc[:, 0]) - 0.25 * math.pi) % (0.5 * math.pi)
    
    # If add_pi_half is provided, it is a tensor that determines if pi/2 is added element-wise
    if add_pi_half is not None:
        yaw += add_pi_half * (0.5 * math.pi)  # Element-wise addition of pi/2 where add_pi_half is 1
...
asset_options = gymapi.AssetOptions()
box_assets = []
random_range = rolloutPara.uncertaintyPara.shape_range
for i in range(num_boxes):
    # Randomize length and width, keep height constant
    length = box_size + random.uniform(random_range[0], random_range[1])  # Randomized length
    width = box_size + random.uniform(random_range[0], random_range[1])   # Randomized width
    height = box_size  # Fixed height
    # Create a box asset with randomized dimensions
    box_asset = gym.create_box(sim, length, width, height, asset_options)
...
# configure env grid
num_envs = 2 + rolloutPara.num_envs # first: target(blue)    second: current(red)    rest: planning(green)
...
box_y = []
box_quat = []
mass_range = rolloutPara.uncertaintyPara.mass_range
mass_tensor = torch.zeros((1 + rolloutPara.num_envs, 3), device=device)
...
for env_i in range(num_envs):
    ...
    if env_i == 0:
        ...
        box_handle = gym.create_actor(env, random.choice(box_assets), box_pose, "box", env_i, 0)
    ...
    # current real env generation
        ...
        for box_i in range(num_boxs):
            ...
            mass_tensor[env_i-1, box_i] = body_props[0].mass
            gym.set_actor_rigid_body_properties(env, box_handle, body_props)
            tmp_box_idxs.append(box_idx)
...
freeze_time = int(0.8 / sim_params.dt)
...
# trajectory tensor
trajectory_tensor = torch.zeros((1 + rolloutPara.num_envs, 300, 7), device=device)
record_counter = -1
current_cost = torch.zeros(1+rolloutPara.num_envs, device=device)
calculate_flag = True


# which grasp point to choose
grasp_choice_tensor = torch.randint(0, 2, (num_envs, )).to(device)

# simulation loop
while not gym.query_viewer_has_closed(viewer):
    counter += 1
    if action_status[1] == 2:
        record_counter += 1
    # step the physics
    gym.simulate(sim)
    ...
    box_pos = rb_states[[sublist[rolloutPara.reducUncertainPara.object_id] for sublist in box_idxs], :3]
    box_rot = rb_states[[sublist[rolloutPara.reducUncertainPara.object_id] for sublist in box_idxs], 3:7]
    # print("box_idxs", box_idxs)
    # print("box_pos, box_rot", box_pos, box_rot)
    target_box_pos = rb_states[[*box_idxs[0]], :3]
    current_box_pos = rb_states[[*box_idxs[1]], :3]
    target_hand_pos = target_box_pos + torch.tensor([[0, 0, 0.15]]).to(device)
    ...
    gripped = (gripper_sep < 0.045+random_range[1]) & (to_box_dist < grasp_offset + 0.5 * box_size)

    grasp_choice_tensor[:] = rolloutPara.actionPara.grasp_id
    # print("grasp_choice_tensor", grasp_choice_tensor)
    yaw_q = cube_grasping_yaw(box_rot, corners, grasp_choice_tensor)
    ...
    grasp_pos[:, 2] = torch.where(above_box, box_pos[:, 2] + grasp_offset*0.9, box_pos[:, 2] + grasp_offset * 2.5)
    # print("to_box_dist[1], action_status[1], gripped[1]", to_box_dist[1], action_status[1], gripped[1])
    ...
    for env_i in range(num_envs):
        ...
        if action_status[env_i] == 0 and to_box_dist[env_i] < 0.12:
    ...
    # print("trajectory_tensor[:, 0, :], dof_pos[1:, :7].squeeze()", trajectory_tensor[:, 0, :], dof_pos[1:, :7].squeeze())
    # record trajectory
    if action_status[1] == 2:
        trajectory_tensor[:, record_counter, :] = dof_pos[1:, :7].squeeze()
    ...
    # # Initialize the dictionary for success rates of each (object_choice, grasp_choice) combination
    # success_rates = {}

    # # Loop through each combination of object_choice (0, 1, 2) and grasp_choice (0, 1)
    # for object_choice in range(3):
    #     for grasp_choice in range(2):
            
    #         # Mask for the current combination of object_choice and grasp_choice
    #         object_mask = (object_choice_tensor[2:] == object_choice)
    #         grasp_mask = (grasp_choice_tensor[2:] == grasp_choice)
    #         combined_mask = object_mask & grasp_mask  # Logical AND to get the specific combination

    #         # Filter success_rate by the combined mask
    #         success_rate_for_combination = success_rate[2:][combined_mask]

    #         # Calculate the success rate percentage for the current combination
    #         if success_rate_for_combination.numel() > 0:  # Avoid division by zero
    #             total_success_rate_for_combination = success_rate_for_combination.float().mean().item() * 100
    #         else:
    #             total_success_rate_for_combination = 0  # No entries for this combination

    #         # Store the success rate in the dictionary with a tuple key
    #         success_rates[(object_choice, grasp_choice)] = total_success_rate_for_combination
    #         print(f"Success rate for object choice {object_choice}, grasp choice {grasp_choice}: "
    #             f"{total_success_rate_for_combination:.2f}% "
    #             f"(Total values: {success_rate_for_combination.numel()})")

    # # Determine the combination with the highest success rate
    # best_combination = max(success_rates, key=success_rates.get)
    # highest_success_rate = success_rates[best_combination]
    # print(f"Best combination: Object choice {best_combination[0]}, Grasp choice {best_combination[1]} "
    #     f"with a success rate of {highest_success_rate:.2f}%")
    ...
    if action_status[1] == 2:
        pos_action[2:, :] = pos_action[1, :]
        
    # calculate the trajectory difference
    if action_status[1] == 3 and calculate_flag:
        calculate_flag = False
        for traj_i in range(1+rolloutPara.num_envs):
            current_cost[traj_i] = calculateTrajectorySimilarity(trajectory_tensor[0].cpu().numpy(), trajectory_tensor[traj_i].cpu().numpy())

        mass_np = mass_tensor.cpu().numpy()
        current_cost_np = current_cost.cpu().numpy()

        # Scatter plot
        plt.figure(figsize=(8, 6))
        plt.scatter(mass_np[1:, rolloutPara.reducUncertainPara.object_id], current_cost_np[1:], label='Mass vs Current Cost', color='blue', alpha=0.7)
        plt.xlabel('Mass')
        plt.ylabel('Difference')
        plt.title('Ground Truth:{mass}'.format(mass = mass_tensor[0, rolloutPara.reducUncertainPara.object_id]))
        plt.legend()
        plt.grid(True)
        plt.show()
