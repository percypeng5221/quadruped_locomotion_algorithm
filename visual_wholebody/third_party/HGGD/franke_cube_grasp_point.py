...
from GraspInference import GraspDetector
...
body_names_to_idx = gym.get_asset_rigid_body_dict(franka_asset)
hand_idx = body_names_to_idx["panda_hand"]
base_idx = body_names_to_idx["panda_link0"]
...
for i in range(num_envs):
    ...
    gym.set_rigid_body_color(env, box_handle, 0, gymapi.MESH_VISUAL, color)
    ...
    # get hand and base handle
    hand_handle = gym.get_actor_rigid_body_handle(env, franka_handle, hand_idx)
    base_handle = gym.get_actor_rigid_body_handle(env, franka_handle, base_idx)
    
    # camera_offset = gymapi.Vec3(0.0, 0.0, 0.1)
    # camera_rotation = gymapi.Quat.from_axis_angle(gymapi.Vec3(1, 1, 1), np.deg2rad(250))
    # gym.attach_camera_to_body(cam_handle, env, hand_handle, gymapi.Transform(camera_offset, camera_rotation), gymapi.FOLLOW_TRANSFORM)
    
    # attach cameras to rigid bodies
    camera_offset = gymapi.Vec3(-0.5, 0.0, 1.0)
    camera_rotation = gymapi.Quat.from_euler_zyx(0, np.pi/4, 0)
    gym.attach_camera_to_body(cam_handle, env, base_handle, gymapi.Transform(camera_offset, camera_rotation), gymapi.FOLLOW_TRANSFORM)
    
    cams.append(cam_handle)

    # obtain camera tensor
    rgb_tensor = gym.get_camera_image_gpu_tensor(sim, env, cam_handle, gymapi.IMAGE_COLOR)
    depth_tensor = gym.get_camera_image_gpu_tensor(sim, env, cam_handle, gymapi.IMAGE_DEPTH)
    # print("Got rgb tensor with shape", rgb_tensor.shape)
    # print("Got depth tensor with shape", depth_tensor.shape)

    # wrap camera tensor in a pytorch tensor
    torch_rgb_tensor = gymtorch.wrap_tensor(rgb_tensor)
    torch_depth_tensor = gymtorch.wrap_tensor(depth_tensor)
    rgb_tensors.append(torch_rgb_tensor)
    ...
    base_idx = gym.find_actor_rigid_body_index(env, franka_handle, "panda_link0", gymapi.DOMAIN_SIM)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if timer > 1 and len(result.translations):
        print(goal_pos, goal_rot)
        print(result[0].translation, result[0].rotation)
    ...
    rgb_image = gym.get_camera_image_gpu_tensor(sim, envs[0], cams[0], gymapi.IMAGE_COLOR)
    rgb_image = gymtorch.wrap_tensor(rgb_image)
    
    depth_image = gym.get_camera_image_gpu_tensor(sim, envs[0], cams[0], gymapi.IMAGE_DEPTH)
    depth_image = gymtorch.wrap_tensor(depth_image)

    rgb_frame_np = rgb_image.cpu().numpy()
    depth_frame_np = depth_image.cpu().numpy() * -1
    
    # print(depth_frame_np)
    rgb_frame = rgb_frame_np[..., :3]
    rgb_frame = (rgb_frame * 255).astype(np.uint8)

    depth_frame = depth_frame_np * 1000 
    if timer == 1:
        result = grasp_detector.inference(rgb_frame, depth_frame, vis_heatmap=True, vis_grasp=True)
    # if len(result.translations):
    #     print("result.translation, result.rotation: ", result[0].translation, result[0].rotation)
    
    normalized_depth = cv2.normalize(depth_frame_np, None, 0, 255, cv2.NORM_MINMAX)

    # Convert the depth image to uint8 format for OpenCV compatibility
    ...
    # depth_frame = (255 * (depth_frame_np / depth_frame_np.max())).astype(np.uint8)

    # Convert depth frame to 3-channel grayscale for OpenCV compatibility
    # depth_frame = cv2.cvtColor(depth_frame, cv2.COLOR_GRAY2BGR)

    # Display the RGB and Depth frames using OpenCV
    cv2.imshow("RGB Frame", rgb_frame)
    # cv2.imshow("Depth Frame", depth_frame_np)
